...
// ✅ Use before responding with presale data


const prisma = new PrismaClient();

/**
 * Ensures we handle 2 scenarios:
 *   - "Early completion" if stage is fully raised early
 *   - "Extension" if stage is incomplete but end time passed
 * Then we shift subsequent stages so none overlap.
 */
// async function syncPresaleStages() {
//   // 1) get all presale stages in ascending order
...
//   // 2) identify the "current" stage (first incomplete) or fallback to last
...
//   // 3) scenario A: stage is fully funded early => end it now
...
//   }
//   // 4) scenario B: stage incomplete & endTime < now => extension
//   else if (currentStage.raised < currentStage.target && currentStage.endTime < now) {
//     const extensionDays = 7; // or any logic
...
//   // 5) shift future stages so each starts when the previous ends
...
export async function GET() {
  try {
    // automatically sync presale stages
    ...
    const wusleRate = currentStage.rate;
    const listingPrice = currentStage.listingPrice;
...
//     const totalWusleSupply = process.env.TOTAL_WUSLE_SUPPLY || "108971250.2492";
//     const liquidityAtLaunch = process.env.LIQUIDITY_AT_LAUNCH || "5,000,000";
